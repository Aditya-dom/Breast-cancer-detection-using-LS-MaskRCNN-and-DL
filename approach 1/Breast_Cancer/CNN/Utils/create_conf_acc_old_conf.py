...
with open('/home/Drive2/aditya/Benign_conf.txt', 'rb') as f:
    benign_confidence = pickle.load(f)
with open('/home/Drive2/aditya/InSitu_conf.txt', 'rb') as f:
    insitu_confidence = pickle.load(f)
with open('/home/Drive2/aditya/Invasive_conf.txt', 'rb') as f:
    invasive_confidence = pickle.load(f)
with open('/home/Drive2/aditya/Normal_conf.txt', 'rb') as f:
    normal_confidence = pickle.load(f)
...
for i in range(no_samples):
    ...
    benign[i][(benign_confidence[i][:]<threshold)] = -1
    ...
    print((benign[i][:]==-1).sum())
    if np.sum(benign_hist[i,:]) == 0:
        print("Reduce threshold requested by Benign",i)
    ...
    if benign_class == 1 or benign_class == 3 or benign_class == 2:
        for j in range(benign_ss[i]):
            shutil.copyfile("/home/Drive2/aditya/Aug_Data/Validation/Benign/"+str(i)+"_"+str(j)+".png","/home/Drive2/aditya/Wrong_Class/Benign/"+str(i)+"_"+str(j)+"_TC_"+str(benign_class)+"_AC_0_PC_"+str(int(benign[i][j]))+".png")
        print(" Benign " + str(i) + " incorrectly classified as " + str(benign_class))
    
    insitu[i][(insitu_confidence[i][:]<threshold)] = -1
    ...
    if np.sum(insitu_hist[i,:]) == 0:
        print("Reduce threshold requested by insitu",i)
    ...
    invasive[i][(invasive_confidence[i][:]<threshold)] = -1
    ...
    if np.sum(invasive_hist[i,:]) == 0:
        print("Reduce threshold requested by invasive",i)
    ...
    normal[i][(normal_confidence[i][:]<threshold)] = -1
    ...
    if np.sum(normal_hist[i,:]) == 0:
        print("Reduce threshold requested by normal",i)
